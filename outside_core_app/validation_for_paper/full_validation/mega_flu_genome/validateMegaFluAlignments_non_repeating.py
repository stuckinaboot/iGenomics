...
print 'alignments file, ref file alignments were simulated from, potentially multi-genome file with no repeats'
...
fileAllNonRepeatingPath = argv[3]
...
with open(refFilePath) as ref:
	for line in ref.readlines():
		...
		if '>' in line and ('(HA)' in line or '(NA)' in line):
			passingSegments.append(line)

with open(fileAllNonRepeatingPath) as fileAll:
	for line in fileAll.readlines():
		line = line.strip('\r').strip('\n')
		if '>' in line and '(HA)' not in line and '(NA)' not in line:
			passingSegments.append(line)
